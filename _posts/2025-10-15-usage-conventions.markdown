...
This blog post shares outputs from one of the work streams at the recent Zarr Summit focusing on how to define, share and apply Zarr usage conventions for application-specific metadata. 
...
These metadata are useful for downstream applications and users, but are not essential to the basic reading and writing of data. 
...
Here's an example of attributes in an array or group metadata file, based on an example from the [v3 core spec](https://zarr-specs.readthedocs.io/en/latest/v3/core/index.html#metadata):
...
Several user communities would like to define conventions for how they use attributes, to achieve interoperability between downstream applications. 
